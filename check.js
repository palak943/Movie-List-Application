...
function onMovieItemClick(movieIndex) {
    ...
    // Check if the selectedMovie exists and has the "Year" property
    if (selectedMovie && selectedMovie.Year) {
      const year = `Year: ${selectedMovie.Year}`;
      const genre = selectedMovie.Genre ? `Genre: ${selectedMovie.Genre}` : 'Genre: N/A';
      const plot = selectedMovie.Plot ? `Plot: ${selectedMovie.Plot}` : 'Plot: N/A';
      ...
      moviePlotElement.textContent = plot;
  
      // Toggle the "hidden" class to show/hide the movie details section
      const movieDetailsSection = document.getElementById('movieDetails');
      movieDetailsSection.classList.toggle('hidden');
    }
  }
  ...
  function onClearSearchButtonClick() {
    ...
    fetchAndDisplayMovies(); // Fetch and display movies with the default search query
    hideMovieDetails(); // Hide the movie details section when clearing the search
  }
...
function onMovieItemClick(movieIndex) {
    ...
    if (!selectedMovie) {
      console.error('Selected movie is null or undefined.');
      return;
    }
...
function updatePagination(totalResults) {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    const textt=document.createTextNode(i);
    pageNumber.className="pagenum";
    pageNumber.appendChild(textt);
